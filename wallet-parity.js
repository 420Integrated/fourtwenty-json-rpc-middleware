...
function createParityWalletMiddleware({ walletMiddleware }){
  ...
  async function defaultAccount (req, res) {
    res.result = await callOnWalletMiddleware({ method: 'fourtwenty_coinbase', params: [] })
  }
  ...
  async function postTransaction (req, res) {
    ...
    setTimeout(async () => {
      try {
        const txHash = await callOnWalletMiddleware({ method: 'fourtwenty_sendTransaction', params: req.params })
  ...
  async function postSign (req, res) {
    ...
    setTimeout(async () => {
      try {
        const sigResult = await callOnWalletMiddleware({ method: 'fourtwenty_sign', params: req.params })
