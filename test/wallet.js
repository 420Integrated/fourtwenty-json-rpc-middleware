...
const BlockTracker = require('fourtwenty-block-tracker')
...
fourtwentySignTest({
  testLabel: 'fourtwenty_sign - valid address',
...
fourtwentySignTest({
  testLabel: 'fourtwenty_sign - invalid address',
...
fourtwentySignTypedDataTest({
  testLabel: 'fourtwenty_signTypedData - valid address',
  address: testAddresses[0],
  accounts: testAddresses.slice(),
  fromAddressIsValid: true,
})
...
fourtwentySignTypedDataTest({
  testLabel: 'fourtwenty_signTypedData - invalid address',
  address: testUnkownAddress,
  accounts: testAddresses.slice(),
  fromAddressIsValid: false,
})
...
personalRecoverTest({
  testLabel: 'g420 kumavis manual I recover',
  // "hello world"
...
personalRecoverTest({
  testLabel: 'g420 kumavis manual II recover',
...
function fourtwentySignTest({ testLabel, address, accounts, fromAddressIsValid }) {
  const { engine, query } = createTestSetup()

  const witnessedMsgParams = []

  const getAccounts = async () => accounts.slice()
  const processFourtwentySignMessage = async (msgParams) => {
    witnessedMsgParams.push(msgParams)
    return testMsgSig
  }
  engine.push(createWalletMiddleware({ getAccounts, processFourtwentySignMessage }))

  test(testLabel, async (t) => {
    try {
      const message = 'haay wuurl'
      const payload = { method: 'fourtwenty_sign', params: [address, message] }
      ...
      if (!fromAddressIsValid && err.message.includes('Invalid parameters: must provide an 420coin address.')) {
...
function fourtwentySignTypedDataTest({ testLabel, address, accounts, fromAddressIsValid }) {
  const { engine, query } = createTestSetup()
  ...
  test(testLabel, async (t) => {
    try {
      ...
      const payload = { method: 'fourtwenty_signTypedData', params: [message, address] }
      const signMsgResponse = await pify(engine.handle).call(engine, payload)
      const signMsgResult = signMsgResponse.result
      if (fromAddressIsValid) {
        t.ok(signMsgResult, 'got result')
...
function transactionTest({ testLabel, txParams, accounts, fromAddressIsValid }) {
  ...
  test(testLabel, async (t) => {
    try {
      const payload = { method: 'fourtwenty_sendTransaction', params: [txParams] }
...
function createTestSetup () {
  ...
  const ganacheQuery = new FourtwentyQuery(ganacheProvider)
