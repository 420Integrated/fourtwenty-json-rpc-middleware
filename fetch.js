...
function checkForHttpErrors (fetchRes) {
  ...
  switch (fetchRes.status) {
    case 405:
      throw fourtwentyErrors.rpc.methodNotFound()
...
function parseResponse (fetchRes, body) {
  ...
  if (fetchRes.status !== 200) {
    throw fourtwentyErrors.rpc.internal({
      message: `Non-200 status code: '${fetchRes.status}'`,
      data: body,
    })
  }
  // check for rpc error
  if (body.error) throw fourtwentyErrors.rpc.internal({
    data: body.error,
  })
...
function createRatelimitError () {
  return fourtwentyErrors.rpc.internal({ message: `Request is being rate limited.` })
}

function createTimeoutError () {
  let msg = `Gateway timeout. The request took too long to process. `
  msg += `This can happen when querying logs over too wide a block range.`
  return fourtwentyErrors.rpc.internal({ message: msg })
}
